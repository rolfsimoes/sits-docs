...
```{r}
# select NDVI band
samples_ndvi <- sits_select(samples_matogrosso_mod13q1, "NDVI")
# select only samples with Cerrado label
samples_cerrado <-
    dplyr::filter(samples_ndvi, label == "Cerrado")
```
...
```{r}
# Obtain a data cube with 23 instances for one year
data_dir <- system.file("extdata/sinop", package = "sitsdata")

# create a raster metadata file based on the information about the files
cube <- sits_cube(source = "LOCAL",
                   satellite = "TERRA",
                   sensor  = "MODIS",
                   name = "Sinop",
                   data_dir = data_dir,
...
cube %>% dplyr::select(type, URL, satellite, sensor)
# get information on the coverage
cube %>% dplyr::select(xmin, xmax, ymin, ymax)
```
...
points <- sits_get_data(cube, file = samples.csv)
